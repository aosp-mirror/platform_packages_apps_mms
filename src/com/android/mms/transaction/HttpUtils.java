    ...
    protected static byte[] httpConnection(Context context, long token,
        ...
        try {
            ...
            if (extraHttpParams != null) {
                ...
                String addendum = MmsConfig.getHttpParamsNaiAddendum();
                String paramList[] = extraHttpParams.split("\\|");
                String nai = null;

                if (!TextUtils.isEmpty(naiKey)) {
                    nai = SystemProperties.get("persist.radio.cdma.nai", null);
                    if (!TextUtils.isEmpty(nai)) {
                        // Some operators use "nai + additional info".
                        // This value could set as mcc/mnc.
                        if (!TextUtils.isEmpty(addendum))
                            nai = nai + addendum;
                ...
                for (String paramPair : paramList) {
                    ...
                    if (splitPair.length == 2) {
                        ...
                        if (value.indexOf(line1Key) >= 0 && line1Number != null) {
                            value = value.replace(line1Key, line1Number);
                        }
                        if (value.indexOf(naiKey) >= 0 && nai != null) {
