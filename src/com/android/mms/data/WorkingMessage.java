...
public class WorkingMessage {
    ...
    */
    ...
    public void send(final String recipientsInUI) {
        ...
        if (spliter.getSMSConversation() != null){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    preSendSmsWorker(spliter.getSMSConversation(), msgText, recipientsInUI);
    ...
    private boolean isPartiallySMSCompliant() {
       ...
       if (   ((mMmsState & FORCE_MMS)== 0) //NO slideshow that will convert the whole message for the whole recipients into MMS
           && ((mMmsState & LENGTH_REQUIRES_MMS)== 0) //DO NOT exceed a particular threshold of SMS truncation that will convert
                                                       //the whole message for the whole recipients into MMS
           && ((mMmsState & HAS_ATTACHMENT)== 0)      //NO attachment that will convert the whole message for
                                                       //the whole recipients into MMS
           && ((mMmsState & HAS_SUBJECT)== 0)) { //NO subject that will convert the whole message for the whole recipients into MMS
        //DO NOT check RECIPIENTS_REQUIRE_MMS to allow email & alias as recipients
        result = true;//Conversation where MMS could not be mandatory to for some recipients : aka phone numbers.
       }
       return result;
    }
